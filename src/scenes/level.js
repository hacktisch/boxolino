...
import { drawSprite, text, hint } from '../engine/draw.js';
...
export class LevelScene {
  ...
  get blocks() {
    const L = this.L;
    return [L.gate, ...L.bosses, ...L.shops, ...L.bags, ...L.walls].map(t => t.block);
  }
  ...
  update(dt, input) {
    ...
    if (input.pressed('Enter')) {
      ...
      else if (boss) this.game.switchTo('arena', { boss: BOSSES.find(b => b.id === boss.id) });
  ...
  hit(t, mult) {
    ...
    if (t.kind === 'bag') {
      ...
      fx.float(`+${xp} XP`, t.x, t.y - t.h, '#27ae60');
      state.bagHits[t.id] = (state.bagHits[t.id] || 0) + 1;
      if (t.unlock && state.bagHits[t.id] === t.unlock.hits && !state.flags.includes(t.unlock.flag)) {
        state.flags.push(t.unlock.flag); state.save(); sound.play('win'); fx.bump(10);
        fx.float(TEXT.newAttack, 480, 300, '#8e44ad', 40);
      }
      ...
      fx.float(`-${dmg}`, t.x, t.y - t.h - 10, '#c0392b');
      if (w.hp <= 0) {
        const lvl = state.wallLevel(t.id), coins = t.coins(lvl);
        fx.float(TEXT.wallBroken, t.x, 300, '#e67e22', 40); fx.float(`+${coins} munten`, t.x, 340, '#b7950b', 30); fx.bump(14);
  ...
  draw(ctx) {
    ...
    for (const b of L.bosses) {
      ...
      text(ctx, `🪙 ${Math.round(def.prizeCoins * Math.pow(def.prizeGrowth, wins))}`, b.x, b.y + (b.sign ? 66 : 32), { size: 15, align: 'center', color: '#b7950b' });
    }
  ...
  drawWall(ctx, t) {
    ...
    text(ctx, `Muur ${lvl}`, t.x, y0 - 40, { size: 20, align: 'center' });
    text(ctx, `${Math.max(0, w.hp)}/${t.hp(lvl)}`, t.x, y0 - 18, { size: 16, align: 'center', color: '#c0392b' });
  }
  drawHints(ctx) {
